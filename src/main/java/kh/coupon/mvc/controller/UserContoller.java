...
public class UserContoller {
	...
	//제휴업체 쿠폰 리스트
		...
		public String coupon_buy(Model model,int member_no, int client_no, int menu_no, String member_name, int coupon_count, RedirectAttributes redirectAttributes) {
			int res = coupon_biz.coupon_buy(member_no,client_no,menu_no,member_name,coupon_count);
			if(res>0) {
		...
		*/
		...
		//---------------------------------이용후기&공지사항 게시판---------------------------------------------------------------
			...
			public String review_board_list(Model model,String board_category) {
				...
				model.addAttribute("list",board_biz.review_list(board_category));
			...
			public String review_detail(Model model,int board_no) {
				board_biz.updateHit(board_no);
				model.addAttribute("dto",board_biz.review_detail(board_no));
			...
			public String review_insert(Model model,BoardDto review_dto,HttpServletRequest request) {
				int res = board_biz.review_insert(review_dto);
				if(res>0) {
					model.addAttribute("list",board_biz.review_list(review_dto.getBoard_category()));
					return "userViews/reviewBoardList";
				} else {
					return "userViews/reviewBoardInsert";
				}
			...
			public String review_update(Model model,BoardDto review_dto,int board_no) {
				int res = board_biz.review_update(review_dto);
				if(res>0) {
					model.addAttribute("dto",board_biz.review_detail(board_no));
					return "userViews/reviewBoardDetail";
				} else {
			...
			public String review_delete(Model model,int board_no,String board_category) {
				board_biz.review_delete(board_no);
				model.addAttribute("list",board_biz.review_list(board_category));
				return "userViews/reviewBoardList";
			}
			...
			public String notice_list(Model model) {
				...
				return "userViews/notice_list";
			}
			
			//사용자 공지사항 디테일 보기
			@RequestMapping("notice_detail")
			public String notice_detail(Model model, int board_no) {
				model.addAttribute("notice_dto",board_biz.board_detail(board_no));
				return "userViews/notice_detail";
			}
